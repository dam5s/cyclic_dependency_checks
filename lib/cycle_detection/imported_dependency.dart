...
class ImportedDependency {
  ...
  static Module _sourceModule(String appPackage, SourceFile source) {
    final components = source.path.split(Path.separator)
      ..removeLast()
      ..removeAt(0)
      ..insert(0, appPackage);

    final path = components.join(Module.separator);

    return Module(path);
  }

  static Module _importedModule(String appPackage, String import, Module sourceModule) {
    final components = import.replaceFirst('package:', '').split(Module.separator)..removeLast();
    return Module.joining(components);
  }
}
